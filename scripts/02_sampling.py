...
def clean_description(s: str, max_chars: int = 800) -> str:
    """Goodreads description often contains HTML. Keep it lightweight."""
    if not s:
        return ""
    s = html.unescape(s)
    s = TAG_RE.sub(" ", s)
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Reservoir-sample English books with description from goodreads_books.json.gz and save parquet."
    )
    p.add_argument("--input", required=True, help="Path to goodreads_books.json.gz")
    p.add_argument("--out_dir", required=True, help="Output directory for sample files")
...
def main():
    ...
    reservoir = []
    seen = 0  # count of eligible records
        ...
        meta_rows.append(
            {
                ...
                "num_pages": int(b.get("num_pages") or 0) if str(b.get("num_pages") or "").isdigit() else None,
